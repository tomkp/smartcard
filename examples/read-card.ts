...
async function main(): Promise<void> {
    ...
    try {
        ...
        if (readerIndex >= readers.length) {
            console.log(
                `Reader index ${readerIndex} out of range. Found ${readers.length} reader(s).`
            );
            return;
        }
        ...
        try {
            ...
            if (response.length >= 2) {
                ...
                if (sw1 === 0x90 && sw2 === 0x00) {
                    ...
                    console.log(
                        `  Command returned: ${sw1.toString(16)} ${sw2.toString(16)}`
                    );
                    console.log(
                        '  (UID read may not be supported by this card type)'
                    );
                }
            }
        } catch (err) {
